...
class MySim:
    ...
    x = h.sim.Param(5)
    mydc = h.sim.Dc(var=x, sweep=h.sim.PointSweep([1]))
    mytran = h.sim.Tran(tstop=11 * h.prefix.PICO)
    delay = h.sim.Meas(analysis=mytran, expr="trig_targ_something")
    # opts = h.sim.Options(reltol=1e-9)

    save_all = h.sim.Save(h.sim.SaveMode.ALL)
    a_path = "."
    include_that_path = hs.Include(a_path)
    fast_lib = h.sim.Lib(path=a_path, section="fast")
