...
from piel.types import Reference, ScalarMetric
...
# Create ScalarMetric Instances

# Chen et al. (2007) Metrics
chen_bandwidth = ScalarMetric(
    min=0.4 * 1e9,  # 0.4 GHz
    max=10 * 1e9,  # 10 GHz
    unit=Hz,
)

chen_noise_figure = ScalarMetric(min=4.4, max=6.5, unit=dB)

chen_power_gain = ScalarMetric(min=11.2, max=12.4, unit=dB)

chen_power_consumption = ScalarMetric(value=12, min=12, max=12, unit=mW)

chen_supply_voltage = ScalarMetric(value=1.8, min=1.8, max=1.8, unit=V)

chen_technology = ScalarMetric(value=180, min=180, max=180, unit=nm)

chen_footprint = ScalarMetric(value=0.42, min=0.42, max=0.42, unit=mm2)

# Liu et al. (2003) Metrics
liu_bandwidth = ScalarMetric(
    min=0.5 * 1e9,  # 0.5 GHz
    max=14 * 1e9,  # 14 GHz
    unit=Hz,
)

liu_noise_figure = ScalarMetric(min=3.2, max=5.4, unit=dB)

liu_power_gain = ScalarMetric(value=10.6, min=10.6, max=10.6, unit=dB)

liu_power_consumption = ScalarMetric(value=52, min=52, max=52, unit=mW)

liu_supply_voltage = ScalarMetric(value=1.3, min=1.3, max=1.3, unit=V)

liu_technology = ScalarMetric(value=180, min=180, max=180, unit=nm)

# Footprint "1.0 x 1.6" mm² is 1.6 mm²
liu_footprint = ScalarMetric(value=1.0 * 1.6, min=1.6, max=1.6, unit=mm2)

# Parvizi et al. (2014) Metrics
parvizi_bandwidth = ScalarMetric(
    min=0.1 * 1e9,  # 0.1 GHz
    max=7 * 1e9,  # 7 GHz
    unit=Hz,
)

parvizi_noise_figure = ScalarMetric(value=5.5, min=5.5, max=5.5, unit=dB)

parvizi_power_gain = ScalarMetric(value=12.6, min=12.6, max=12.6, unit=dB)

parvizi_power_consumption = ScalarMetric(value=0.75, min=0.75, max=0.75, unit=mW)

parvizi_supply_voltage = ScalarMetric(value=0.5, min=0.5, max=0.5, unit=V)

parvizi_technology = ScalarMetric(value=90, min=90, max=90, unit=nm)

parvizi_footprint = ScalarMetric(value=0.23, min=0.23, max=0.23, unit=mm2)

# Asgaran et al. (2006) Metrics
asgaran_bandwidth = ScalarMetric(
...
asgaran_noise_figure = ScalarMetric(value=3.4, min=3.4, max=3.4, unit=dB)

asgaran_power_gain = ScalarMetric(value=11.45, min=11.45, max=11.45, unit=dB)

asgaran_power_consumption = ScalarMetric(value=4, min=4, max=4, unit=mW)

asgaran_supply_voltage = ScalarMetric(value=0.5, min=0.5, max=0.5, unit=V)

asgaran_technology = ScalarMetric(value=180, min=180, max=180, unit=nm)

# Footprint "0.950 x 0.900" mm² is 0.855 mm²
asgaran_footprint = ScalarMetric(value=0.950 * 0.900, min=0.855, max=0.855, unit=mm2)
