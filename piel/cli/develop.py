...
@develop.command(name="generate-poetry2nix-flake", help="Generates the poetry2nix flake.")
def generate_poetry2nix_flake(args=None):
    ...
    try:
        echo_and_check_subprocess(
            ["nix", "flake", "init", "--template", "github:nix-community/poetry2nix", "--experimental-features",
             "nix-command flakes"])
        ...
        if return_path("flake.nix").exists():
            ...
            raise Exception("Failed to generate the poetry2nix flake and none was found in the root directory.s")


@develop.command(name="build-piel-cachix", help="Activates the specific piel nix environment.")
def build_piel_cachix_command(args=None):
    ...
    if platform.system() == "Windows":
        ...
        echo_and_check_subprocess(
            ["nix", "build", "--extra-experimental-features", "nix-command", "--extra-experimental-features",
             "flakes", "--show-trace"],
